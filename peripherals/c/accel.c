...
static __INLINE uint8_t accel_reg_read(uint8_t reg)
{
  // ADD CODE
	...
	accel_CSN_low();
	spiTx(ACCEL_SPI_BASE, ((uint8_t*)(&i)), 2, ((uint8_t*)(&r)));
	accel_CSN_high();
  return r[1];
}
...
static __INLINE void accel_reg_write(uint8_t reg, uint8_t data)
{
	...
	i[1] = data;
	accel_CSN_low();	
	spiTx(ACCEL_SPI_BASE, ((uint8_t*)(&i)), 3, ((uint8_t*)(&r)));
	accel_CSN_high();
}
...
int16_t accel_read_x(void)
{
  ...
  // ADD CODE
  return (accel_reg_read(ACCEL_OUTX_H_XL) << 8) | (accel_reg_read(ACCEL_OUTX_L_XL));
...
{
  return (accel_reg_read(ACCEL_OUTY_H_XL) << 8) | (accel_reg_read(ACCEL_OUTY_L_XL));
...
{
 return (accel_reg_read(ACCEL_OUTZ_H_XL) << 8) | (accel_reg_read(ACCEL_OUTZ_L_XL));
